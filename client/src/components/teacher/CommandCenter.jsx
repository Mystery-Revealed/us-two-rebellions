...
// → approve names → watch live status and one class-wide accuracy average →
// download the PDF → End Session (deletes everything from memory). Two
// Rebellions is single-role solo (no pick), so students start on their own as
// soon as they're approved — no pairing, and one class group instead of a
// per-pick split.
...
const GROUP = 'class';
const TEKS = 'TEKS 8.15B, 8.5A, 8.17B, 8.1A';
...
export default function CommandCenter() {
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Two Rebellions: A Tale of Two Governments · {TEKS}</p>
  ...
  const cls = roster?.classAccuracy?.[GROUP] || { count: 0, average: 0 };
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Two Rebellions · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student takes command right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        <div className="class-summary one">
          <div className="summary-stat wide">
            <div className="big-number">{cls.count ? `${cls.average}%` : '—'}</div>
            <div className="muted">Class average — {students.length} students — {cls.count} completed</div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="btn-row">
          <button className="btn" onClick={() => downloadPdf(roster, cls)}>⬇ Download PDF report</button>
        </div>
...
function downloadPdf(roster, cls) {
  if (!roster) return;
  ...
  doc.setFontSize(16);
  doc.text('Two Rebellions: A Tale of Two Governments — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  const completed = roster.students.filter((s) => s.status === 'completed').length;
  doc.text(
    `Class code: ${roster.joinCode}   ·   Date: ${date}   ·   Students: ${roster.students.length}   ·   Completed: ${completed}   ·   Class average: ${cls.count ? `${cls.average}%` : '—'}`,
    14, 23
  );
  ...
  doc.text('8th Grade U.S. History · Two Rebellions: A Tale of Two Governments · TEKS 8.15B, 8.5A, 8.17B, 8.1A', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`two-rebellions_${roster.joinCode}_${date}.pdf`);
}
