...
// and inspects the emit instructions it returns. No sockets involved. Two
// Rebellions is single-role solo (no pick, no branch, no AI rival), so these
// focus on the solo lifecycle, the one class-wide group, and the accuracy path.
import test from 'node:test';
import assert from 'node:assert/strict';
import { GameManager } from '../src/GameManager.js';
import game, { phasesFor, SIDE } from '../src/games/usTwoRebellions.js';
...
test('the default game is Two Rebellions', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  assert.equal(manager.registry.get(joinCode).gameId, 'us-two-rebellions');
});
...
test('a student joins, the match begins on join, and all-right earns 100% and Chief Historian', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  ...
  assert.equal(end.length, 1, 'match ends after 12 orders');
  assert.equal(end[0].payload.you.accuracy, 100);
  assert.equal(end[0].payload.you.ending.key, 'chief');
  assert.match(end[0].payload.you.debrief, /Constitution/i, 'debrief present');
